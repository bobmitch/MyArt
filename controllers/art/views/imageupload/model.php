...
foreach ($_FILES["file-upload"]["error"] as $key => $error) {
    if ($error == UPLOAD_ERR_OK) {
		...
		$query = "insert into media (width, height, title, alt, filename, mimetype, created_by) values (?,?,?,?,?,?,?)";
		$stmt = $pdo->prepare($query);
		$in_db_ok = $stmt->execute(array($file->width, $file->height, $title, $alt, $file->filename, $file->mimetype, CMS::Instance()->user->id));
		if ($in_db_ok) {
