...
if ($resetemail) {
	...
	CMS::Instance()->queue_message('If your email was associated with a user, you should receive a message with further instructions shortly.','success',Config::$uripath . '/');
}
...
if ($resetkey) {
	...
	if ($password1 && $password2) {
		if ($password1 != $password2) {
			CMS::Instance()->queue_message('Passwords did not match.','danger', Config::$uripath . '/?resetkey=' . $resetkey);	
		}
		else {
			...
			if ($reset_user_exists) {
				...
				if (!$reset_user->remove_reset_key()) {
					CMS::Instance()->queue_message('Error removing reset key.', 'error', Config::$uripath."/");
				}
				if ($reset_user->update_password ($password1)) {
					CMS::Instance()->queue_message('Password changed for ' . $reset_user->username,'success', Config::$uripath . '/');	
				}
				else {
					CMS::Instance()->queue_message('Unable to reset password. Please contact the system administrator.','danger', Config::$uripath . '/?resetkey=' . $resetkey);		
				}
			}
			else {
				// no matching user for resetkey found or resetkey is outdated
				CMS::Instance()->queue_message('Invalid reset key or reset key is too old.','danger', Config::$uripath . '/?resetkey=' . $resetkey);	
			}
		}
	}
		...
		<div id="login" class='container '>
			...
			<?php elseif ($view=='resetpassword'):?>
				<form class='' submit='' action='' method="POST">
					...
					<p class="help"><a href='<?php echo Config::$uripath;?>/'>Login</a></p>

				</form>
			<?php elseif ($view=='newpassword'):?>
				<form class='' submit='<?php echo Config::$uripath . '/?view=newpassword>resetkey=' . $resetkey?>' action='' method="POST">
